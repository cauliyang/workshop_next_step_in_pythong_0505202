...
Let's apply TDD to build a simple shopping cart:

```python
# First write a failing test (RED phase)
def test_add_item():
    cart = ShoppingCart()
...
Now let's extend with another test:

```python
# RED phase - new test for discount
...
## Alternative TDD Demo: Statistics Function

A simpler example for a short workshop:
